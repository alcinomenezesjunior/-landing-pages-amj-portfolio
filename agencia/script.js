...
function initCountdown() {
  ...
  /* WhatsApp com UTM */
  function initWhatsAppUTM() {
    const waLinks = $$('.js-wa, .wa-float, a[href*="wa.me"], a[data-cta-type="whatsapp"]');
    waLinks.forEach(link => {
      link.addEventListener('click', (e) => {
        const source = link.dataset.waSource || 'landing_page';
        pushToDataLayer({
          event: 'whatsapp_click',
          wa_source: source,
          wa_phone: link.dataset.phone || '+351926698959'
        });
      });
    });
  }
  ...
  function initExitIntent() {
    ...
    $('.exit-popup__close', popup)?.addEventListener('click', hidePopup);
    $('.exit-popup__secondary', popup)?.addEventListener('click', hidePopup);
    $('.exit-popup__overlay', popup)?.addEventListener('click', hidePopup);

    // Track CTA clicks
    $$('[data-exit-cta]', popup).forEach(btn => {
      btn.addEventListener('click', () => {
        pushToDataLayer({
          event: 'exit_popup_cta_click',
          cta_type: btn.dataset.exitCta
        });
        hidePopup();
      });
    });
  ...
  function initCertLightbox() {
    ...
    $$('.certificate-lightbox-trigger').forEach(trigger => {
